...
if (Platform.isForge()) {
  $ResourcePackLoader = Java.loadClass(
    'net.minecraftforge.resource.ResourcePackLoader'
  )
} else {
  $PlatformImpl = Java.loadClass(
    'dev.architectury.platform.fabric.PlatformImpl'
  )
  $ModNioResourcePack = Java.loadClass(
    'net.fabricmc.fabric.impl.resource.loader.ModNioResourcePack'
  )
}

const $Collectors = Java.loadClass('java.util.stream.Collectors')
const $ServerPacksSource = Java.loadClass(
  'net.minecraft.server.packs.repository.ServerPacksSource'
)
const $FallbackResourceManager = Java.loadClass(
...
function loadPack(type, modId) {
  ...
  if (modId == 'minecraft') {
    ...
    let resourcePack = Platform.isForge()
      ? loadModForge(type, modId)
      : loadModFabric(type, modId)
...
function loadModFabric(type, modId) {
  ...
  let resourcePack = $ModNioResourcePack.create(
    mod.getModId(),
    modContainer,
    null,
    type,
